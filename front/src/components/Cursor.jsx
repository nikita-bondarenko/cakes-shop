...
export function AnimatedCursor({
    ...
    const {isNav} = useAppSelector(state => state.ui)
    const {cursorColor} = useAppSelector(state => state.ui)
    const [color, setColor] = useState(cursorColor)

    useEffect(() => {

        isNav ? setTimeout(() => setColor(cursorColor), 400) : setColor(cursorColor)
    }, [cursorColor])
    ...
    React.useEffect(() => {
        const clickables = document.querySelectorAll(
            'a, text-number[type="submit"], text-number[type="image"], label[for], select, button, .link'
        )
    ...
    const styles = {
        ...
        cursorInner: {
            ...
            transition: 'opacity 0.15s ease-in-out, transform 0.25s ease-in-out, background-color .25s ease-in-out'
        },
        cursorOuter: {
            zIndex: 999,
            position: 'fixed',
            ...
            transition: 'opacity 0.15s ease-in-out, transform 0.15s ease-in-out, background-color .25s ease-in-out'
        }
    }
